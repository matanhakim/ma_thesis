...
df <- read_muni_new("data/municipalities/2018.xlsx")
...
# df_2 <- read_muni_old_var("data/municipalities/2013.xls", mahoz, 3, 3)
...
```{r}
df_muni_names <- read_muni_id(include_names = TRUE) |> 
...
df_yishuvim_names <- read_yishuv(yishuv_name, 1) |> 
  left_join(read_yishuv(yishuv_name_eng_1, 25), join_by(yishuv_id)) |> 
  mutate(
...
read_organizations <- function() {
  bind_rows(
    ...
    arrange(tax_id, yishuv_name) |> 
    distinct(tax_id, .keep_all = TRUE) |> 
    left_join(df_yishuvim_names, join_by(yishuv_name))
}
```
...
This adds the missing yishuvim names to the relevant data frame.

```{r}
df_yishuvim_names_2 <- tibble(
  yishuv_name = c(
    "תל אביב - יפו",
    "דנ גליל עליון",
    "מעין ברוך", 
...
df_yishuvim_names <- df_yishuvim_names |> 
  bind_rows(df_yishuvim_names_2)
```

This function matches every organization supported by the ministry of culture with its municiaplity id. this leaves us with a municipality id for every budget support of the ministry of culture for every organization in every year. missing values include yishuvim not part of any municipality, like Mikveh Israel and the airport, and budgets that do not go to organiztions, mostly prizes for indivduals.
