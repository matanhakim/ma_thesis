...
read_elect_pct <- function() {
  ...
  read_elect_muni() |>  
    ...
    group_by(muni_id) |> 
    summarize(
      elec_likud_pct = 100 * sum(halikud) / sum(good_votes),
      elec_coal_pct = 100 * sum(
        yahadut_hatorah +
        habait_hayehudi +
        kulanu +
        israel_beytenu +
...
```{r}
read_ses_2013 <- function() {
  df <- read_excel("data/indices/t02.xls", skip = 6)
  ...
  df |> 
    ...
    mutate(
      muni_id = as.character(muni_id),
      muni_id = case_when(
        (muni_status == "0" | muni_status == "99") ~ fix_yishuv_id(muni_id),
        TRUE ~ str_pad(muni_status, width = 2, side = "left", pad = "0")
      )
    ) |> 
    select(!muni_status)
