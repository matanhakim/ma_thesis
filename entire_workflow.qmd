...
read_ses_2013 <- function(url){
  ...
  read_excel("data_files/municipalities/t02.xls", skip = 6) |> 
    slice(2:256) |>
    select(
      muni_status = 1,
      muni_id = 2,
...
read_peri_2004 <- function() {
  ...
  read_excel("data_files/municipalities/24_08_160t2.xls", skip = 7) |> 
    select(
      muni_id = 1,
      peri_2004_i = 9,
      peri_2004_r = 10,
...
read_amutot <- function(){
  ...
  read_excel("data_files/organizations/דוח חודשי גיידסטאר.xlsx", sheet = 2) |> 
    select(
      tax_id = 2,
      yishuv_name = 14
    ) |> 
...
read_companies <- function(){
  read_csv("data_files/organizations/companies.csv") |> 
    select(
      tax_id = 1,
      yishuv_name = 13
    ) |> 
