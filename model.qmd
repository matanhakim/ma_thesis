...
Because we are now working with the original SELA budget variable, Let's examine the most correlated variables.
...
anv_21
anv_p_value <- anv_21 %>% tidy() %>%  slice_tail() %>% pull(p.value)
...
mdl2 %>% tidy()
```
The second model has an R-squared value of `r rsq_2` that represents an R-squared change of `r rsq_2 - rsq_1` with a confidence level p-value of `r anv_p_value`.
...
anv_32
anv_p_value <- anv_32 %>% tidy() %>%  slice_tail() %>% pull(p.value)
