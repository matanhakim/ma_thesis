...
get## National priority settlements decided by the Israeli government
...
The companies data currently works, but some companies have their city name in the street name variable. need to think how to resolve this.
...
ses_sa_2008_url <- "https://www.cbs.gov.il/he/mediarelease/doclib/2013/087/24_13_087t6.xls" # Initializing the url for 2008 CBS statistical areas data
...
budget_culture_2014_df <- get_budget_data_open(culture_2014_url, 2014) %>% # Reading and calculating the total budget of the ministry of culture in 2014 by municipality
  match_budget_org() %>%
  add_muni_id_na(org_id, muni_id) %>%
  group_by(muni_id) %>% 
  summarise(across(c(budget_approved, budget_paid), ~ sum(.x, na.rm = TRUE)))
